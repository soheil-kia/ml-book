# Sample 1.9: Diabetes Elastic Net Regression
...
elastic_regression = make_pipeline(PolynomialFeatures(degree),
                                 ElasticNet(alpha=1e-3,l1_ratio=0.5))
elastic_regression.fit(X_train, y_train)
elastic_predict = elastic_regression.predict(X_test)
print("Elastic Net Regression MSE: %.2f"
      % mean_squared_error(y_test, elastic_predict))
